...
class Story_Teller(object):
    def __init__(
        self, V=0, L=0, O=0, isExport=False, isDebug=True, contentFile="StoryText.yaml"
    ):
        self.V = V  # V value
        self.L = L  # L value
        self.O = O  # O value
        ...
        self.tipFlag = (
            False  # Whether O get tip of "AI testers" from AI colleagues at Step1
        )
    ...
    def InputContent(self):
        if path.isfile(self.contentFile):
            ...
            err_en = (
                "Error: Fail to find the content text file '" + self.contentFile + "'."
            )
    ...
    def SaveProcess(self, *filename):
        ...
        try:
            ...
            err_en = (
                "Error: Fail to save current game process to file '" + filename + "'."
            )
    ...
    def ExportStep(self, stepID, text, *filename):
        ...
        if self.isDebug:
            ...
            content += (
                "V=" + str(self.V) + "\tL=" + str(self.L) + "\tO=" + str(self.O) + "\n"
            )
        content += text
        content += "\n"
        print(content)
    ...
    def RecognizeDecision(self, inputDecision):
        ...
        if (
            inputDecision == "0"
            or inputDecision.upper() == "N"
            or inputDecision.upper() == "F"
        ):
            return False
        elif (
            inputDecision == "1"
            or inputDecision.upper() == "Y"
            or inputDecision.upper() == "T"
        ):
            return True
        else:
