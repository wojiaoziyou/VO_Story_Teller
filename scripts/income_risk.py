...
class Income_Risk(object):
    def __init__(
        ...
        orders_safe_min=8,
        orders_safe_max=35,
        risk_rate_safe=0.005,
        orders_init=10,
        orders_adding=0,
        orders_is_expected=False,
    ):
    ...
    def Risk_rate_actual(self, orders_actual):
        ...
        self.risk_rate_dangerous = (
            self.risk_rate_safe + (self.time_safe - self.time_expected) / 1000
        )
    ...
    def Total_income(self, orders_actual, orders_expected):
        ...
        if orders_actual < orders_expected:
            return (
                (8 - np.power(50, time_actual - self.time_expected)) * orders_actual
            ) / 10
        else:
